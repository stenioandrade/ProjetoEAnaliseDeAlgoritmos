...
int main(int argc, char **argv){
	...
	int i, j, ctrl=0, auxi=0, auxqntp=0, auxprint=0, auxtotp=0, flag=0, napareceu=1;
	...
	while(ctrl<totp){
		...
		while(napareceu){
			for(i=0;i<qntp;i++){
					...
					if(pacote[i+auxqntp].nump == auxi) napareceu = 0;
		...
		/*		
		if(auxtotp > 0){
		//printf("if auxtotp > 0 %d\n",auxtotp);
			for(i=0;i<qntp;i++){
				//printf("%d\n",x++);
				fscanf(input,"%d %d",&pacote[i+auxqntp].nump,&pacote[i+auxqntp].tamp);
				for(j=0;j<pacote[i+auxqntp].tamp;j++) fscanf(input," %X",&pacote[i+auxqntp].bytp[j]);
				auxtotp--;
				printf("%d\n",pacote[i+auxqntp].nump);
			}
			auxqntp += qntp;
		}
		*/
		...
		while(auxqntp > 0){
		//printf("pacote: %d auxi: %d auxqntp: %d\n",pacote[0].nump,auxi,auxqntp);
